...
- `enabled=false`：不核算（totalCost 记 0，UI 与 PDF 不展示成本）
- 每零件分摊成本一律按面积占比
...
- **PDF = 给人看**：交付客户、沟通确认、存档。含零件标注（名称/尺寸/数量）、统计信息（板材数/利用率/成本）、美观排版、可打印、水印/公司信息。
...
- **字体按实际字符集加载**（不依赖界面语言）：渲染前扫描整份文档文本（词条 + 用户输入的零件名/备注），按字符归组加载字体——拉丁组（Noto Sans，含西里尔）/ SC / JP / KR / Thai 共 5 组；运行时子集化（subset-font，纯 JS+wasm，本地跑不联网）只嵌用到的字符
- **字体分发策略**：
